...
const getApiData = async (page) => {
    try {
        let apiUrl = ` https://api.rawg.io/api/games?key=${CLAVE_PI}&page=${page}`;
...
const getApiGenres = async () => {
    let url = `https://api.rawg.io/api/genres?key=${CLAVE_PI}`;
...
const getApiVgByQuery = async (name) => {
    try {
        const apiInfo = await axios.get(`https://api.rawg.io/api/games?search=${name}&key=${CLAVE_PI}`);
        const apiData = await apiInfo.data.results.map(game => {
            return {
                img: game.background_image,
                name: game.name,
...
const getApiVgById = async (id) => {
    ...
    let url = `https://api.rawg.io/api/games/${id}?key=${CLAVE_PI}`;
